...
X.sub("init", function() {
    ...
    function onLogin(type, cred) {
	    ...
	    function onGotNonce(respText) {
	        ...
	        var captcha= (cred.captchaToken) ? "&captchaToken="+cred.captchaToken : "";
	        X.post("/user/login?username="+cred.username+"&password="+hash+"&nonce="+resp.nonce+"&cnonce="+cn+captcha, null, cred.onLoginResponse);
	    }
